...
printf("READING INTO LINKNAME\n");
		...
		sprintf(insQuery, "INSERT INTO CompXZ VALUES (0, %llu, '%s', '%s', '%s', %d, %llu, %ld, '%s', '%c')", archive_id, real_filename, membername_nopath, membername_path, tmp_blocknumber, ((block_offsets->blocklocations)[tmp_blocknumber]).position, tmp_blockposition, header.size, header.typeflag[0]);
		if(mysql_query(con, insQuery)) {
			printf("Insert error:\n%s\n", mysql_error(con));
			printf("%s\n", insQuery);
			dberror = 1;
