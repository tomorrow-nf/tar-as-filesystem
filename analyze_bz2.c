...
printf("READING INTO LINKNAME\n");
		...
		*membername_ptr = '\0'; // truncate path string
		...
		sprintf(insQuery, "INSERT INTO Bzip2_files VALUES (0, %llu, '%s', '%s', '%s', %d, %llu, %ld, '%s', '%c', '%c', %ld, %ld, %ld)", archive_id, real_filename, membername_file, membername_path, tmp_blocknumber, ((block_offsets->blocklocations)[tmp_blocknumber]).position, tmp_blockposition, header.size, header.typeflag[0], dirflag, strtol(header.mode, NULL, 8), strtol(header.uid, NULL, 8), strtol(header.gid, NULL, 8));
		if(mysql_query(con, insQuery)) {
			printf("Insert error:\n%s\n", mysql_error(con));
			printf("%s\n", insQuery);
			dberror = 1;
